...
- **实例配置异步合并落盘（防抖 Save）**：`Add/UpdateInstance/Remove` 此前
  每次都同步「全量序列化 + 写临时文件 + fsync + rename」整个
  `instances.json`，且被 `saveMu` 串行化——1 万次创建实例的压测吞吐从
  185 req/s 衰减到 21 req/s（文件越大越慢），总耗时 3 分 54 秒。改为
  热路径只改内存并打脏标记，后台循环按防抖窗口（默认 500ms）合并写盘、
  优雅关停兜底落盘后，同样的 1 万次 POST 实测 **0.6 秒（约 18000 req/s，
  含收尾一次落盘 47ms）**。代价：进程被强杀（断电 / kill -9）时最多丢
  一个窗口内的配置变更；优雅关停（SIGTERM/SIGINT）不丢。
