...
| 错误 | `status != 200` 时 `data` 为错误消息字符串；**HTTP 状态码与 `body.status` 一致** |
...
> **安全限制**：`source.address` 仅允许 http/https 协议；以下目标一律拒绝：
> 环回（127.0.0.1/::1）、未指定、链路本地（含云元数据 169.254.x.x）、组播、
> 广播、指向本节点自身的地址，以及 **RFC1918 内网地址（10/8、172.16/12、
> 192.168/16，含 IPv6 ULA fc00::/7）**。重定向目标与远端响应中的直连地址
> 同样逐跳校验。
>
> **信任假设与配置**：内网地址默认拒绝——若认证失守（弱 apikey / 配对码泄漏），
> 放行内网意味着节点可被用作内网扫描跳板（SSRF）。集群 **LAN 节点间直传**是
> 该接口的设计用途，需要放行内网时由管理员显式配置：
>
> ```bash
> irix-node -transfer-allow-cidr 192.168.0.0/16,10.0.0.0/8
> ```
>
> `-transfer-allow-cidr` 为逗号分隔 CIDR 列表；环回 / 链路本地 / 本机地址
> 属于硬性拒绝，任何配置都不可放行。**信任边界**：配置放行即默认信任这些
> 网段内的节点——请仅放行自己管辖的 LAN 网段，避免使用 0.0.0.0/0。

**票据类型**：下载票据与上传票据严格区分（`/download/*` 与 `/upload/*` 互不可用）；
实例级下载票据绑定申请时的单文件，只能下载该文件（防同目录越权读取与目录浏览），
集群 / 快照下载票据按设计保留目录范围。
...
> 说明：当前 `irix-node` Go 源码不在本仓库（无 `node/` 目录），上述 P0–P2 均需在 `irix-node` 侧实现；MCSM 侧受其「无法节点互联」限制，本文档不为其规划新增 API。
